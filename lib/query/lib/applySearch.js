...
function getSort(searchSort) {
  if (!searchSort) {
    return {};
  }

  // We use sort in the $search stage when using an indexed top field 
  // https://www.mongodb.com/docs/atlas/atlas-search/sort/
  const { searchScore, ...rest } = searchSort;

  const sort = { ...rest };

  if (searchScore !== undefined) {
    sort.score = { $meta: "searchScore", order: searchScore };
  }
